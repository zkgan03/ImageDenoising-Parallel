...
*/
void SequentialWaveletThreshold::visuShrink(
	const cv::Mat& input,
	cv::Mat& output,
	int level,
	...
	*/
	...
	cv::Mat dwtOutput;
	SequentialHaarWavelet::dwt(input, dwtOutput, level); // dwt
...
*/
void SequentialWaveletThreshold::neighShrink(
	const cv::Mat& input,
	cv::Mat& output,
	int level,
	int windowSize
...
void SequentialWaveletThreshold::applyNeighShrink(
	const cv::Mat& coeffs,
	cv::Mat& output,
	double threshold,
	int halfWindow
...
*/
void SequentialWaveletThreshold::modineighShrink(
	const cv::Mat& input,
	cv::Mat& output,
	int level,
	int windowSize
...
void SequentialWaveletThreshold::applyModiNeighShrink(
	const cv::Mat& coeffs,
	cv::Mat& output,
	double threshold,
	int halfWindow
...
*/
void SequentialWaveletThreshold::bayesShrink(
	const cv::Mat& input,
	cv::Mat& output,
	int level,
	SequentialThresholdMode mode
	...
	/*
		1. Apply wavelet transform to the input image
	*/
	SequentialHaarWavelet::dwt(input, output, level); // dwt
...
void SequentialWaveletThreshold::applyBayesShrink(
	cv::Mat& coeffs,
	double sigmaNoise,
	SequentialThresholdMode mode
) {
...
*/
double SequentialWaveletThreshold::calculateUniversalThreshold(
	cv::Mat& highFreqBand
) {
...
double SequentialWaveletThreshold::calculateSigma(
	cv::Mat& coeffs
) {
...
float SequentialWaveletThreshold::sign(float x)
{
	float res = 0;
	if (x == 0)
	{
...
float SequentialWaveletThreshold::soft_shrink(float d, float threshold)
{
	return std::abs(d) > threshold ? std::copysign(std::abs(d) - threshold, d) : 0.0;
}

float SequentialWaveletThreshold::hard_shrink(float x, float threshold)
{
	return std::abs(x) > threshold ? x : 0.0;
}

float SequentialWaveletThreshold::garrot_shrink(float x, float threshold)
{
